...
async def _handle_bank_statement(message: Message, bot: Bot, tmp_path: Path, file_name: str) -> None:
    ...
    try:
        ...
        extraction_prompt = (
            ...
            "  * ВАЖНО: 'С Карт X' / 'С счёта X' / 'С карты X' — деньги ПРИШЛИ с того счёта НА счёт выписки\n"
            "    → счёт X: transfer_out; счёт выписки: transfer_in\n"
            "  * ВАЖНО: 'На Карт X' / 'На счёт X' / 'На карту X' — деньги УШЛИ с счёта выписки НА тот счёт\n"
            "    → счёт выписки: transfer_out; счёт X: transfer_in\n"
            "  * Пример: выписка по account_main, операция 'С Карт Депозита +10,000' →\n"
            "    {account: account_2, tx_type: transfer_out, amount: 10000} +\n"
            "    {account: account_main, tx_type: transfer_in, amount: 10000}\n"
            "  * Пример: выписка по account_main, операция 'На Карт Депозита -10,000' →\n"
