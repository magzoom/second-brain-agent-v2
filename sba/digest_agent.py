...
async def _send_digest_tool(args: dict[str, Any]) -> dict[str, Any]:
    ...
    _digest_sent = True  # mark as called — prevents fallback even if delivery fails
    if not _notifier:
        logger.error("send_digest: _notifier is None — message will NOT be sent")
        return _ok("Notifier not initialized")
    text = _md_to_html(args.get("text", ""))
    MAX_TG = 4096
    ...
    try:
        ...
        logger.error(f"send_digest: Telegram delivery failed: {e}")
        return _ok(f"Ошибка отправки: {e}")
...
5. Сформируй одно красивое сообщение с эмодзи. После КАЖДОГО пункта обязательно ставь ссылку на источник в формате: <a href="https://t.me/...">Канал</a>
...
async def run_digest(notifier, config: dict) -> None:
    ...
    if not _digest_sent:
        ...
        if last_result and len(last_result) > 50:
            # Try to find the actual digest content (starts with 🌅 emoji)
            content = last_result
            idx = last_result.find("🌅")
            if idx > 0:
                content = last_result[idx:]
            await notifier.send_message(_md_to_html(content[:4000]))
