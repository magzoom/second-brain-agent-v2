...
2. Раздел ПОГОДА — если данные переданы в ПОГОДА: — выведи одной строкой. Если данных нет — пропусти раздел.
3. Раздел ДАЙДЖЕСТ — отбери лучшее из постов по категориям:
...
🌤 <b>ПОГОДА:</b> [одна строка если данные есть]
...
async def _fetch_weather(config: dict) -> str:
    """Fetch current weather from wttr.in. Uses last known GPS location if available, falls back to config city."""
    import urllib.request as _ur, json as _json

    # Prefer GPS coordinates saved by the bot when user shares location
    loc_file = Path.home() / ".sba" / "last_location.json"
    if loc_file.exists():
        try:
            loc = _json.loads(loc_file.read_text())
            location = f"{loc['lat']},{loc['lon']}"
            area_label = None  # will be resolved from API response
        except Exception:
            location = config.get("digest", {}).get("location", "Almaty")
            area_label = location
    else:
        location = config.get("digest", {}).get("location", "Almaty")
        area_label = location
    ...
    try:
        ...
        t_min = today_fc["mintempC"]
        t_max = today_fc["maxtempC"]
        if area_label is None:
            area_label = data.get("nearest_area", [{}])[0].get("areaName", [{}])[0].get("value", location)
        return (
            f"🌤 {area_label}: {desc}, {temp}°C (ощущается {feels}°C)\n"
            f"   Сегодня {t_min}–{t_max}°C, влажность {humidity}%"
        )
    except Exception as e:
...
async def _prefetch_data(config: dict, hours_back: int = 16) -> tuple[str, str, str]:
    """Pre-fetch Telegram posts, Google Tasks and weather before calling the agent."""
    # --- Weather ---
    weather_text = await _fetch_weather(config)
    ...
    return posts_text, tasks_text, weather_text
...
async def run_digest(notifier, config: dict) -> None:
    ...
    posts_text, tasks_text, weather_text = await _prefetch_data(config, hours_back=16)
